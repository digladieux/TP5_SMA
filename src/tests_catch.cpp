...
TEST_CASE("Json", "[json]")
{
    ...
    if (!file.fail())
    {
        ...
        CHECK(0.7 == j["config2"]["chanceMale"]);
...
TEST_CASE("Exception", "[exception]")
{
    ...
    std::vector<unsigned int> vector_character = {2, 3, 8, 9, 15, 16};
    std::vector<unsigned int> vector_map = {2, 3, 8, 9, 15, 16};
    std::vector<unsigned int> vector_map_error = {80, 66};
    std::vector<unsigned int> vector_character_error = {80, 89};
...
TEST_CASE("Game", "[game]")
{
    ...
    Game game(vector_map, vector_character, 1, Date(1, 1, 60), 2, 0);
    game.run(10);
}
...
TEST_CASE("Menu", "[menu]")
{
    ...
    system("clear");
    std::cout << "The game will be played 3 times for 3 differents strategies for the characters" << std::endl ;
    std::cout << "1 : The character go to the closest collection point of his speciality" << std::endl;
    std::cout << "2 : The character go to the closest collection point and the ressource will be the lowest of the townhall" << std::endl;
    std::cout << "3 : The character go to the closest collection " << std::endl;
    sleep(5) ;
    Game game(map_choice,character_choice, config_choice, Date(1, 1, 60), display_choice, 0);
